...
const userByAccountNum = async (req, res, next) => {
    try {
        ...
        const user = await User.getByAccountNum(num)
...
const userByIdentityNum = async (req, res, next) => {
    try {
        const { num } = req.params
        // THIS PROCESS CAN ALSO GET DATA FROM CACHE, FIND FROM JSON PARSED REDIS KEY
        const user = await User.getIdentityNum(num)
        return res.status(200).json({ data: user })
    } catch (error) {
        next(error)
    }
...
const getAllUser = async (req, res, next) => {
    try {
        const USER_CACHE = JSON.parse(await cache.get(USER_CACHE_KEY))
        if (USER_CACHE) return res.status(200).json({ data: USER_CACHE })

        const allUser = await User.getAll()
        await cache.set(USER_CACHE_KEY, JSON.stringify(allUser));
...
const createUser = async (req, res, next) => {
    try {
        ...
        await cache.del(USER_CACHE_KEY);
...
const editUser = async (req, res, next) => {
    try {
        ...
        const { id } = req.params
        await User.edit(editUserData, id)
        await cache.del(USER_CACHE_KEY);
